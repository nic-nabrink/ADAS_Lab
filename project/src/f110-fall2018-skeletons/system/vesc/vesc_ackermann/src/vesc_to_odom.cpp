  ...
  use_servo_cmd_(true), publish_tf_(false), x_(0.0), y_(0.0), yaw_(0.0)
{
  ...
  if (use_servo_cmd_) {
    ...
    imu_data_sub_ = nh.subscribe("/imu_data", 10, &VescToOdom::imuDataCallback, this);
  }
}

void VescToOdom::imuDataCallback(const sensor_msgs::Imu::ConstPtr& imu_data)
{
    tf::Quaternion imu_quat;
    imu_quat.setX(imu_data->orientation.x);
    imu_quat.setY(imu_data->orientation.y);
    imu_quat.setZ(imu_data->orientation.z);
    imu_quat.setW(imu_data->orientation.w);
...
void VescToOdom::vescStateCallback(const vesc_msgs::VescStateStamped::ConstPtr& state)
{
  ...
  odom->twist.covariance[0] = 0.001; ///< x
  odom->twist.covariance[7] = 0.001; ///< y
